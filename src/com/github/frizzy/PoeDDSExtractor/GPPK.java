...
public class GPPK {
    ...
    private static Logger LOGGER = Logger.getLogger( GPPK.class.getPackageName() );
    ...
    static final String GPPK_TOOL = "ExtractBundledGGPK3.exe";

    /**
     * The File object of LibGPPK exe tool.
     */
    ...
    /**
     *
     * @param gppkPath Path to the directory containing the LibGPPK tools.
    ...
    public Map < File, List < String > > extractEverything ( final Path contentPath, final Path outputPath, File uiimagesTxt ) {
        ...
        int processed = 0;
        DDSProgressHook.setStep( "Extracting .dds files" );
        DDSProgressHook.setValues( 0, gppkFiles.size() );
        ...
        DDSProgressHook.setStep( "Gathering all textures" );
        ...
        for ( File ddsFile : ddsFiles.keySet( ) ) {
            ...
            List < String > allTextures = GPPKUtils.getAllTexturesFor( uiimagesTxt , internalPath );
            allFiles.put( ddsFile , allTextures );
            LOGGER.log( Level.INFO, "Got all textures for: " + internalPath );

            texturesProcessed++;
            DDSProgressHook.updateCurrentValue( texturesProcessed );
        }
    ...
    private Optional < File > extractFile ( Path contentPath, Path outputPath, String wantedFile ) throws IOException {
        ...
        if ( wantedFile.contains( ".dds" ) ) {
            temp = wantedFile.replaceAll( "/", "_" ).replace( ".dds", "" );
        } else if ( wantedFile.contains( ".txt" ) ) {
            temp = wantedFile.replaceAll( "/", "_" ).replace( ".txt", "" );
        }

        File outputDir = new File( outputPath.toString() + File.separator + temp );
        Path path = Path.of( outputDir + File.separator + "path.txt" );

        if ( overwrite && outputDir.exists() || !outputDir.exists() ) {
            ...
            CommandLine commandLine = new CommandLine( gppkExe );
            ...
            if ( exitCode == 0 ) {
                ...
                if ( files != null ) {
                    for ( File f : files ) {
                        ...
                        LOGGER.log( Level.INFO, "Subbed: " + subbed + ", Wanted File: " + wantedFile  );
     ...
     */
    private Optional <File> getGPPKExe ( String gppkPath ) {
        ...
        if ( file.isDirectory() ) {
            ...
            if ( files != null ) {
                for ( File f : files ) {
                    if ( f.getName().contains( GPPK_TOOL ) ) {
                        return Optional.of( f );
                    }
                }
            }
